...
def _zilf_db_conn():
    import sqlite3
    os.makedirs(os.path.dirname(_ZILF_DB_PATH), exist_ok=True)
    conn = sqlite3.connect(_ZILF_DB_PATH)
...
def _require_dashboard_access(request: Request) -> None:
    ...
    if not token:
        return
    provided = (request.headers.get("X-Dashboard-Token", "") or "").strip()
    if not provided or provided != token:
        raise HTTPException(status_code=401, detail="Dashboard token invalid or missing")
...
async def dashboard_meta(request: Request) -> dict:
    token_required = bool(os.environ.get("DASHBOARD_ACCESS_TOKEN", "").strip())
    if token_required:
        _require_dashboard_access(request)
    return {"token_required": token_required}
...
async def dashboard_summary(request: Request) -> dict:
    ...
    try:
        ...
        dau = int((conn.execute(
            "SELECT COUNT(DISTINCT user_id) AS c FROM model_logs WHERE date(created_at) = date('now')"
        ).fetchone() or {"c": 0})["c"])
        mau = int((conn.execute(
            "SELECT COUNT(DISTINCT user_id) AS c FROM model_logs WHERE created_at >= datetime('now','-30 day')"
        ).fetchone() or {"c": 0})["c"])
        avg_latency_ms_24h = conn.execute(
            "SELECT AVG(latency_ms) AS v FROM model_logs WHERE created_at >= datetime('now','-1 day')"
        ).fetchone()
        avg_latency = float(avg_latency_ms_24h["v"]) if (avg_latency_ms_24h and avg_latency_ms_24h["v"] is not None) else None
        revenue_idr = total_users * 20000
        return {
...
async def dashboard_users(request: Request, limit: int = 200) -> dict:
    _require_dashboard_access(request)
    ...
    try:
        ...
        items = [{"id": r["id"], "name": r["name"], "email": r["email"], "created_at": r["created_at"]} for r in rows]
        return {"items": items}
    finally:
        conn.close()


async def dashboard_latency_timeseries(request: Request, days: int = 14) -> dict:
    _require_dashboard_access(request)
    ...
    try:
        rows = conn.execute(
            """
            SELECT substr(created_at, 1, 10) AS day, AVG(latency_ms) AS avg_latency_ms
            FROM model_logs
        ...
        values = [float(r["avg_latency_ms"]) if r["avg_latency_ms"] is not None else 0.0 for r in rows]
        return {"labels": labels, "values": values}
    finally:
        conn.close()


async def dashboard_security_summary(request: Request) -> dict:
    _require_dashboard_access(request)
    conn = _zilf_db_conn()
    try:
        rows = conn.execute(
            "SELECT severity, COUNT(1) AS c FROM security_audit GROUP BY severity"
        ).fetchall()
        counts = {str(r["severity"]).lower(): int(r["c"]) for r in rows}
        order = ["critical", "high", "medium", "low"]
        labels = order
        values = [counts.get(k, 0) for k in order]
        pii = conn.execute(
            "SELECT COUNT(1) AS c FROM security_audit WHERE pii_detected = 1"
        ).fetchone()
        pii_events = int(pii["c"]) if pii else 0
        return {"labels": labels, "values": values, "pii_events": pii_events}
    finally:
        conn.close()
